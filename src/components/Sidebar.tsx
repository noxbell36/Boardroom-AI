...
export default function Sidebar({ currentView, setCurrentView }: SidebarProps) {
  ...
  return (
    <aside className="w-[240px] bg-white border-r border-slate-200 flex flex-col shrink-0 h-full z-20">
      <div className="p-6 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium uppercase tracking-wider">Investment Tool</p>
          </div>
        </div>
      </div>
      
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        <div className="px-3 mb-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-4">Main Menu</div>
        {menuItems.map((item) => {
      ...
      <div className="p-4 border-t border-slate-100">
        ...
        <div className="mt-4 px-3 flex items-center gap-3">
          ...
          <div className="overflow-hidden text-left">
            <p className="text-[13px] font-bold truncate text-slate-800">김지원 예비 심사역</p>
            <p className="text-[11px] text-slate-500 truncate mt-0.5">전략기획 / 경영관리 / 재무</p>
          </div>
        </div>
      </div>
    </aside>
